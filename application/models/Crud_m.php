...
Class Crud_m extends CI_Model {
    ...
    public function get_user_log()
    {
        ...
        $second_date = date('Y-m-d H:i:s', strtotime('-5 minutes'));
        $this->db->from($this->user_log);
        $this->db->where('waktu <=', $first_date);
        $this->db->where('waktu >=', $second_date);
    ...
    function tampil_status_setuju()
    {
        $this->db->select('*');
        $this->db->from('kelas_pengganti');
